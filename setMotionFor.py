...
print("docName and Action", documentName, action)
# window size
seq_length = 30
# recoding time
motion_time = 10
...
while camera.isOpened():
    ...
    # print
    print(documentName, action, data.shape)
    
    # save data in dataset folder (numpy)
    np.save(os.path.join('dataset', f'raw_{documentName}_{action}'), data)
    ...
    print(documentName, action, full_seq_data.shape)
    
    # save data in dataset folder (full_seq_data array)
    np.save(os.path.join('dataset', f'seq_{documentName}_{action}'), full_seq_data)
